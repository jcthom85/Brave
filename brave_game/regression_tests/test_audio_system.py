...
class AudioSystemFilesTests(unittest.TestCase):
    ...
    def test_webclient_template_and_menu_wire_audio_ui(self):
        ...
        self.assertIn("ctx.createMediaElementSource(audio)", audio_source)
        self.assertIn("mediaSource.connect(outputGain)", audio_source)
        self.assertIn("outputGain.connect(busNode)", audio_source)
        self.assertIn("dataArray.fill(0)", default_out_source)
