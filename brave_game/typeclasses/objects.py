...
from world.arcade import get_reward_definition
...
class ArcadeCabinet(Object):
    """Reusable arcade cabinet object with local games, scores, and prizes."""

    def at_object_creation(self):
        super().at_object_creation()
        self.locks.add("get:false();call:false();puppet:false()")
        if self.db.brave_entity_kind is None:
            self.db.brave_entity_kind = "arcade"
        if self.db.brave_arcade_games is None:
            self.db.brave_arcade_games = []
        if self.db.brave_arcade_price is None:
            self.db.brave_arcade_price = 1
        if self.db.brave_arcade_scores is None:
            self.db.brave_arcade_scores = {}
        if self.db.brave_arcade_rewards is None:
            self.db.brave_arcade_rewards = {}

    def get_available_games(self):
        """Return the cabinet's configured game keys."""

        return [game_key for game_key in (self.db.brave_arcade_games or []) if game_key]

    def get_game_price(self, _game_key=None):
        """Return the silver cost to start a run."""

        try:
            return max(0, int(self.db.brave_arcade_price or 1))
        except (TypeError, ValueError):
            return 1

    def get_leaderboard(self, game_key):
        """Return the local leaderboard for one game."""

        scores = dict(self.db.brave_arcade_scores or {})
        return list(scores.get(game_key, []))

    def get_reward_definition(self, game_key):
        """Return the configured reward definition for one game, if any."""

        return get_reward_definition(self, game_key)
