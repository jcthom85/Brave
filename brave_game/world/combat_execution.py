...
from world.data.character_options import ABILITY_LIBRARY
...
def _execute_warrior_ability(encounter, character, ability_key, ability_name, target, derived, level, allies, _enemies):
    if ability_key == "strike":
        if not encounter._roll_hit(derived["accuracy"], target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} glances off {target['key']}.")
            encounter._add_threat(character, 4)
            return
        damage = encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=4)
        encounter._damage_enemy(character, target, damage, extra_text=" A heavy blow lands.")
    ...
    if ability_key == "shieldbash":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} slams wide of {target['key']}.")
            encounter._add_threat(character, 5)
            return
        damage = encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=2)
        encounter._damage_enemy(character, target, damage, extra_text=" The impact staggers the target.")
        if target["hp"] > 0:
    ...
    if ability_key == "tauntingblow":
        if not encounter._roll_hit(derived["accuracy"] + 1, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} misses {target['key']} but still draws attention.")
            encounter._add_threat(character, 8)
            return
        damage = encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=3)
        encounter._damage_enemy(character, target, damage, extra_text=" The taunt lands as hard as the steel.")
        if target["hp"] > 0:
    ...
    if ability_key == "laststand":
        ...
        encounter._heal_character(character, character, amount)
        state = encounter._get_participant_state(character)
...
def _execute_ranger_ability(encounter, character, ability_key, ability_name, target, derived, level, _allies, enemies):
    if ability_key == "quickshot":
        if not encounter._roll_hit(derived["accuracy"] + 10, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} misses {target['key']}.")
            encounter._add_threat(character, 3)
            return
        damage = encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=3)
        encounter._damage_enemy(character, target, damage)
        return
    ...
    if ability_key == "aimedshot":
        if not encounter._roll_hit(derived["accuracy"] + 6, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key} takes the shot but {target['key']} slips the line.")
            encounter._add_threat(character, 4)
            return
    ...
    if ability_key == "snaretrap":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} snaps shut in the wrong place.")
            encounter._add_threat(character, 3)
            return
        encounter._damage_enemy(
            character,
    ...
    if ability_key == "volley":
        ...
        for enemy in enemies:
            ...
            if not encounter._roll_hit(derived["accuracy"] - 2, enemy["dodge"]):
                encounter.obj.msg_contents(f"{character.key}'s {ability_name} misses {enemy['key']}.")
                continue
            damage = encounter._weapon_damage(derived["attack_power"], enemy["armor"], bonus=1)
            encounter._damage_enemy(character, enemy, damage)
        if not fired:
    ...
    if ability_key == "barbedarrow":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} scrapes past {target['key']} without purchase.")
            encounter._add_threat(character, 3)
            return
        encounter._damage_enemy(character, target, encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=4))
        if target["hp"] > 0:
    ...
    if ability_key == "rainofarrows":
        ...
        for enemy in enemies:
            ...
            if not encounter._roll_hit(derived["accuracy"], enemy["dodge"]):
                encounter.obj.msg_contents(f"{character.key}'s {ability_name} misses {enemy['key']}.")
                continue
            encounter._damage_enemy(character, enemy, encounter._weapon_damage(derived["attack_power"], enemy["armor"], bonus=4))
            if enemy["hp"] > 0:
...
def _execute_cleric_ability(encounter, character, ability_key, ability_name, target, derived, level, allies, enemies):
    if ability_key == "heal":
        ...
        encounter._heal_character(character, target, amount)
        return

    if ability_key == "smite":
        if not encounter._roll_hit(derived["accuracy"], target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} fails to connect with {target['key']}.")
            encounter._add_threat(character, 3)
            return
    ...
    if ability_key == "blessing":
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 6, variance=2, divisor=4))
    ...
    if ability_key == "renewinglight":
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 18, variance=5, divisor=2))
    ...
    if ability_key == "sanctuary":
        ...
        for ally in allies:
            ...
            state["guard"] = max(state.get("guard", 0), guard_value)
            encounter._save_participant_state(ally, state)
            encounter._heal_character(character, ally, heal_amount)
    ...
    if ability_key == "cleanse":
        ...
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 5, variance=2, divisor=4))
        return
    ...
    if ability_key == "radiantburst":
        ...
        for enemy in list(encounter.get_active_enemies()):
            fired = True
            if not encounter._roll_hit(derived["accuracy"] + 2, enemy["dodge"]):
                encounter.obj.msg_contents(f"{character.key}'s {ability_name} fails to catch {enemy['key']} in the flare.")
                continue
            bonus = 3 + (4 if "undead" in enemy.get("tags", []) else 0)
            encounter._damage_enemy(character, enemy, encounter._spell_damage(derived["spell_power"], enemy["armor"], bonus=bonus))
        if not fired:
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} finds no targets.")
        return

    if ability_key == "guardianlight":
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 22, variance=5, divisor=2))
        target_state = encounter._get_participant_state(target)
...
def _execute_paladin_ability(encounter, character, ability_key, ability_name, target, derived, level, allies, enemies):
    if ability_key == "holystrike":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} skids off {target['key']} without purchase.")
            encounter._add_threat(character, 4)
            return
        ...
        encounter._damage_enemy(character, target, damage, extra_text=extra_text)
    ...
    if ability_key == "judgement":
        if not encounter._roll_hit(derived["accuracy"] + 3, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} fails to pin {target['key']} in place.")
            encounter._add_threat(character, 4)
            return
    ...
    if ability_key == "handofmercy":
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 15, variance=4, divisor=4))
        cleared = encounter._clear_one_harmful_effect(target)
        if cleared:
    ...
    if ability_key == "rebukeevil":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} leaves only ringing light in the air.")
            encounter._add_threat(character, 4)
            return
        bonus = 3 + (6 if "undead" in target.get("tags", []) else 0)
        encounter._damage_enemy(character, target, encounter._spell_damage(derived["spell_power"], target["armor"], bonus=bonus))
...
def _execute_mage_ability(encounter, character, ability_key, ability_name, target, derived, _level, _allies, enemies):
    if ability_key == "firebolt":
        if not encounter._roll_hit(derived["accuracy"] + 4, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} flies wide of {target['key']}.")
            encounter._add_threat(character, 3)
            return
    ...
    if ability_key == "frostbind":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} breaks apart before it can hold {target['key']}.")
            encounter._add_threat(character, 3)
            return
        damage = encounter._spell_damage(derived["spell_power"], target["armor"], bonus=1)
        encounter._damage_enemy(character, target, damage, extra_text=" Frost locks around its limbs.")
        if target["hp"] > 0:
            target["bound_turns"] = max(target.get("bound_turns", 0), 1)
            encounter._save_enemy(target)
    ...
    if ability_key == "arcspark":
        if not encounter._roll_hit(derived["accuracy"] + 5, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} crackles harmlessly past {target['key']}.")
            encounter._add_threat(character, 3)
            return
        primary_damage = encounter._spell_damage(derived["spell_power"], target["armor"], bonus=3)
        encounter._damage_enemy(character, target, primary_damage)
        other_enemies = [enemy for enemy in enemies if enemy["id"] != target["id"]]
        if other_enemies:
            secondary = other_enemies[0]
            splash = max(1, primary_damage // 2)
            encounter._damage_enemy(character, secondary, splash, extra_text=" Arc spark leaps to a second target.")
        return
    ...
    if ability_key == "staticfield":
        for enemy in enemies:
            encounter._damage_enemy(character, enemy, encounter._spell_damage(derived["spell_power"], enemy["armor"], bonus=1))
            if enemy["hp"] > 0:
    ...
    if ability_key == "icelance":
        if not encounter._roll_hit(derived["accuracy"] + 4, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} shatters wide of {target['key']}.")
            encounter._add_threat(character, 3)
            return
    ...
    if ability_key == "meteorsigil":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} detonates just off the mark.")
            encounter._add_threat(character, 4)
            return
        encounter._damage_enemy(
            character,
        ...
        for enemy in [enemy for enemy in enemies if enemy["id"] != target["id"]]:
            ...
            encounter._damage_enemy(character, enemy, splash, extra_text=" Meteor fire splashes across the line.")
...
def _execute_rogue_ability(encounter, character, ability_key, ability_name, target, derived, level, _allies, _enemies):
    if ability_key == "stab":
        if not encounter._roll_hit(derived["accuracy"] + 8, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} slips past {target['key']} without finding anything vital.")
            encounter._add_threat(character, 3)
            return
    ...
    if ability_key == "backstab":
        if not encounter._roll_hit(derived["accuracy"] + 10, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} never finds the angle on {target['key']}.")
            encounter._add_threat(character, 4)
            return
        openings = 0
        if target.get("marked_turns", 0) > 0:
        ...
        encounter._damage_enemy(character, target, damage, extra_text=" The blade lands exactly where the target is weakest.")
        return

    if ability_key == "poisonblade":
        if not encounter._roll_hit(derived["accuracy"] + 4, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} nicks only air.")
            encounter._add_threat(character, 3)
            return
        encounter._damage_enemy(character, target, encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=3))
        if target["hp"] > 0:
    ...
    if ability_key == "cheapshot":
        if not encounter._roll_hit(derived["accuracy"] + 6, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} never finds soft ground beneath {target['key']}.")
            encounter._add_threat(character, 4)
            return
        encounter._damage_enemy(character, target, encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=4))
        if target["hp"] > 0:
    ...
    if ability_key == "shadowstep":
        if not encounter._roll_hit(derived["accuracy"] + 12, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} leaves only a blur behind {target['key']}.")
            encounter._add_threat(character, 4)
            return
        bonus = 5 + encounter._consume_stealth_bonus(character)
        encounter._damage_enemy(character, target, encounter._weapon_damage(derived["attack_power"], target["armor"], bonus=bonus))
        state = encounter._get_participant_state(character)
        state["guard"] = max(state.get("guard", 0), 3 + level)
        encounter._save_participant_state(character, state)
        return
    ...
    if ability_key == "eviscerate":
        if not encounter._roll_hit(derived["accuracy"] + 4, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} tears through nothing but momentum.")
            encounter._add_threat(character, 5)
            return
        bonus = 7
        if target.get("marked_turns", 0) > 0:
...
def _execute_druid_ability(encounter, character, ability_key, ability_name, target, derived, level, allies, enemies):
    if ability_key == "thornlash":
        if not encounter._roll_hit(derived["accuracy"] + 3, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} whips through empty air around {target['key']}.")
            encounter._add_threat(character, 3)
            return
        lash_bonus = 2
        if target.get("marked_turns", 0) > 0 or target.get("bound_turns", 0) > 0:
        ...
        encounter._damage_enemy(character, target, damage, extra_text=" Thorned force drags across the target.")
        if target["hp"] > 0:
            target["marked_turns"] = max(target.get("marked_turns", 0), 2)
            encounter._save_enemy(target)
    ...
    if ability_key == "entanglingroots":
        if not encounter._roll_hit(derived["accuracy"] + 2, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} clutches only empty ground.")
            encounter._add_threat(character, 3)
            return
        encounter._damage_enemy(character, target, encounter._spell_damage(derived["spell_power"], target["armor"], bonus=2))
        if target["hp"] > 0:
            target["bound_turns"] = max(target.get("bound_turns", 0), 1)
            encounter._save_enemy(target)
        return
    ...
    if ability_key == "moonfire":
        if not encounter._roll_hit(derived["accuracy"] + 3, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} gutters out before it reaches {target['key']}.")
            encounter._add_threat(character, 3)
            return
    ...
    if ability_key == "livingcurrent":
        encounter._heal_character(character, target, encounter._scaled_heal_amount(derived, 13, variance=4, divisor=3))
        secondary_targets = [
            ally
            for ally in allies
    ...
    if ability_key == "rejuvenationgrove":
        for ally in allies:
            encounter._heal_character(character, ally, encounter._scaled_heal_amount(derived, 10, variance=3, divisor=4))
    ...
    if ability_key == "wrathofthegrove":
        if not encounter._roll_hit(derived["accuracy"] + 3, target["dodge"]):
            encounter.obj.msg_contents(f"{character.key}'s {ability_name} churns the ground but misses {target['key']}.")
            encounter._add_threat(character, 4)
            return
