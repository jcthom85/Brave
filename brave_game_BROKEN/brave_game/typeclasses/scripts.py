...
class BraveEncounter(Script):
    ...
    def _enemy_action_timing(self, enemy):
        ...
        if template_key in {"road_wolf", "goblin_cutter", "goblin_slinger", "old_greymaw", "miretooth", "tower_archer", "mag_clamp_drone"}:
            return normalize_atb_profile({"windup_ticks": 1, "recovery_ticks": 1, "telegraph": True})
        if template_key == "ruk_fence_cutter":
    ...
    def _enemy_counterplay_roles(self, enemy, timing=None):
        """Return which broad reaction roles matter for an enemy telegraph."""

        timing = dict(timing or self._enemy_action_timing(enemy))
    ...
    def _enemy_counterplay_text(self, enemy, timing=None):
        """Return concise player guidance for one enemy telegraph."""

        roles = self._enemy_counterplay_roles(enemy, timing=timing)
        if roles == ["guard"]:
            return "Get a guard in place before it lands."
        if roles == ["interrupt", "guard"]:
            return "Interrupt it or guard the hit cleanly."
        if roles == ["interrupt"]:
            return "Interrupt it before it lands."
        return ""

    def _enemy_action_label(self, enemy):
        template_key = (enemy or {}).get("template_key")
        return {
            "road_wolf": "Lunging Bite",
            "goblin_cutter": "Fence-Hook Rush",
            "goblin_slinger": "Cracking Shot",
            "ruk_fence_cutter": "Fence-Cleaver",
    ...
    def _enemy_telegraph_message(self, enemy):
        ...
        body = {
            "road_wolf": f"{enemy['key']} bunches low for {label}.",
            "goblin_cutter": f"{enemy['key']} squares up for {label}.",
            "goblin_slinger": f"{enemy['key']} whirls up a stone for {label}.",
            "ruk_fence_cutter": f"{enemy['key']} drags the axe back for {label}.",
            "old_greymaw": f"{enemy['key']} lowers into the brush, gathering for {label}.",
            "miretooth": f"{enemy['key']} disappears into the reeds and lines up {label}.",
            "tower_archer": f"{enemy['key']} draws a careful bead for {label}.",
            "mag_clamp_drone": f"{enemy['key']} locks its rig and charges {label}.",
            "sir_edric_restless": f"{enemy['key']} raises his blade for {label}.",
            "foreman_coilback": f"{enemy['key']} routes static through the frame for {label}.",
            "captain_varn_blackreed": f"{enemy['key']} shifts his stance and sets up {label}.",
            "grubnak_the_pot_king": f"{enemy['key']} heaves the cauldron high for {label}.",
            "hollow_lantern": f"{enemy['key']} swells with drowned light and prepares {label}.",
        }.get((enemy or {}).get("template_key"), f"{enemy['key']} prepares an attack.")
        hint = self._enemy_counterplay_text(enemy)
        if hint:
            body = f"{body} {hint}"
        return f"|y{body}|n"
    ...
    def _handle_enemy_atb_state(self, enemy):
        ...
        if state.get("phase") == "ready":
            timing = self._enemy_action_timing(enemy)
            action = {
                "kind": "enemy_attack",
                "enemy_id": enemy["id"],
                "label": self._enemy_action_label(enemy),
                "counterplay_roles": self._enemy_counterplay_roles(enemy, timing=timing),
                "counterplay_text": self._enemy_counterplay_text(enemy, timing=timing),
            }
