...
def _class_icon(class_key, fallback="swords"):
    return str(CLASSES.get(class_key, {}).get("icon") or fallback)


PRIMARY_STAT_ORDER = ("strength", "agility", "intellect", "spirit", "vitality")


def _format_stat_bonus_line(bonuses):
    parts = []
...
def _combine_bonus_maps(*bonus_maps):
    totals = {}
    for bonus_map in bonus_maps:
        for stat, amount in (bonus_map or {}).items():
            if not amount:
                continue
...
def _format_class_opening_abilities(class_data):
    progression = class_data.get("progression", [])
    opening = [name for unlock_level, name in progression if unlock_level <= 1]
    followup = next((name for unlock_level, name in progression if unlock_level > 1), None)
    lines = []
    if opening:
        lines.append(f"Starts with: {', '.join(opening)}")
    if followup:
        lines.append(f"First unlock: {followup}")
    return lines
...
def build_chargen_view(account, state, *, error=None):
    ...
    class_icon = _class_icon(state.get("class"))
    ...
    if state.get("class"):
        chips.append(_chip(class_name, class_icon, "muted"))
    ...
    sections = [
        _section(
            ...
            items=[
                ...
                _pair("Class", class_name, class_icon),
            ],
            span="wide",
        )
    ]
    ...
    if step_key == "menunode_welcome":
        ...
        for race_key, race_data in RACES.items():
            race_lines = [race_data["summary"], f"Perk: {race_data['perk']}"]
            bonus_line = _format_stat_bonus_line(race_data.get("bonuses", {}))
            if bonus_line:
                race_lines.append(f"Bonuses: {bonus_line}")
            trait_line = _format_stat_bonus_line(race_data.get("trait_bonuses", {}))
            if trait_line:
                race_lines.append(f"Trait: {trait_line}")
            race_entries.append(
                _entry(
                    race_data["name"],
        ...
        for class_key in VERTICAL_SLICE_CLASSES:
            ...
            class_lines = [class_data["summary"]]
            class_lines.extend(_format_class_opening_abilities(class_data))
            class_entries.append(
                _entry(
                    class_data["name"],
                    meta=class_data["role"],
                    lines=class_lines,
                    background_icon=_class_icon(class_key),
                    command=class_key,
                    chips=[_chip("Current", "check_circle", "good")] if state.get("class") == class_key else [],
                )
            )
        sections.append(_section("Classes", "swords", "entries", items=class_entries, span="wide", variant="class-catalog"))
        ...
        race_lines = [race_data.get("perk", "No perk found.")]
        bonus_line = _format_stat_bonus_line(race_data.get("bonuses", {}))
        if bonus_line:
            race_lines.append(f"Bonuses: {bonus_line}")
        trait_line = _format_stat_bonus_line(race_data.get("trait_bonuses", {}))
        if trait_line:
            race_lines.append(f"Trait: {trait_line}")
        class_lines = [class_data.get("summary", "No class summary found.")]
        class_lines.extend(_format_class_opening_abilities(class_data))
        sections.append(
            _section(
                "Highlights",
                ...
                items=[
                    ...
                    _entry(
                        ...
                        lines=class_lines,
                        background_icon=_class_icon(state.get("class")),
                    ),
                ],
            )
        )
...
def _build_journal_quest_entry(character, quest_key, *, tracked_key=None, nearby_npcs=None, detailed=False):
    ...
    reward_tip = definition.get("reward_tip")
    next_step = definition.get("next_step")
    completion_reaction = definition.get("completion_reaction")
    ...
    if detailed:
        ...
        if state.get("status") == "active":
            if reward_tip:
                lines.append(f"Reward tip: {reward_tip}")
            if next_step:
                lines.append(f"Next lead: {next_step}")
        elif completion_reaction:
            lines.append(f"Aftermath: {completion_reaction}")
    ...
    if next_objective:
        lines.append(f"Next lead: {_format_objective_progress(next_objective)}")
    else:
        lines.append(definition["summary"])
    if reward_text:
        lines.append(f"Reward: {reward_text}")
    ...
    return _entry(
        ...
        actions=actions,
        command=command,
        chips=[_chip(f"{state.get('status', 'active').replace('_', ' ').title()}", "assignment_ind", "accent" if quest_key == tracked_key else "muted")],
    )
...
def build_combat_view(encounter, character):
    ...
    def available_reaction_labels(role):
        labels = []
        seen = set()
        for action in list(combat_actions.get("abilities", [])) + list(combat_actions.get("items", [])):
            if action.get("reaction_role") != role or not action.get("enabled"):
                continue
    ...
    def counterplay_summary(action):
        role_labels = {
            "interrupt": "Interrupt",
            "guard": "Guard",
            "cleanse": "Cleanse",
        ...
        parts = []
        for role in list(action.get("counterplay_roles") or []):
            labels = available_reaction_labels(role)
            if labels:
                parts.append(f"{role_labels.get(role, role.title())}: {', '.join(labels[:3])}")
        if parts:
            return " · ".join(parts)
        return str(action.get("counterplay_text") or "").strip()
    ...
    for participant in ordered_participants:
        ...
        party_entries.append(
            _entry(
                ...
                background_icon=_class_icon(getattr(participant.db, "brave_class", None)),
                chips=status_chips,
                meters=meters,
    ...
    for enemy in enemies:
        ...
        if phase == "winding":
            pressure_text = f"{display_name}: {active_action.get('label', 'Attack')}"
            summary = counterplay_summary(active_action)
            if summary:
                pressure_text += f" · {summary}"
            pressure_items.append(_item(pressure_text, icon="priority_high"))
    ...
    sections = []
    if pressure_items:
        sections.append(_section("Pressure", "priority_high", "list", items=pressure_items, variant="urgent"))
    sections.extend(
        [
...
def build_combat_victory_view(
    ...
    *,
    ...
    chapter_close_messages = [message for message in progress_messages if message.lower().startswith("chapter complete:")]
    town_reaction_messages = [message for message in progress_messages if message.lower().startswith("town reaction:")]
    aftermath_messages = [message for message in progress_messages if message.lower().startswith("aftermath:")]
    trophy_messages = [message for message in progress_messages if message.lower().startswith("trophy added to the hall:")]
    reward_tip_messages = [message for message in progress_messages if message.lower().startswith("reward tip:")]
    next_front_messages = [
        message
        for message in progress_messages
        if message.lower().startswith(("new fronts open:", "choose a focus:", "next lead:", "lead:"))
    ]
    highlighted_messages = (
        level_up_messages
        + chapter_close_messages
        + town_reaction_messages
        + aftermath_messages
        + trophy_messages
        + reward_tip_messages
        + next_front_messages
    )
    other_progress_messages = [message for message in progress_messages if message not in highlighted_messages]
    ...
    chapter_close_lines.extend(chapter_close_messages)
    chapter_close_lines.extend(town_reaction_messages)
    if chapter_close_lines:
        sections.append(
            _section(
                "Chapter Close",
                "emoji_events",
                "lines",
                lines=chapter_close_lines,
                variant="receipt",
            )
        )
    if aftermath_messages:
        sections.append(
            _section(
                "Aftermath",
                "flag",
                "lines",
                lines=aftermath_messages,
                variant="receipt",
            )
        )
    if loot_items_list:
    ...
    if trophy_messages:
        sections.append(
            _section(
                ...
                "workspace_premium",
                "lines",
                lines=trophy_messages,
                variant="receipt",
            )
        )
    if reward_tip_messages:
        sections.append(
            _section(
                "Use Your Take",
                "inventory_2",
                "lines",
                lines=reward_tip_messages,
                variant="receipt",
            )
        )
    ...
    if next_front_messages:
        sections.append(
            _section(
                "Where Next",
                "north_east",
                "lines",
                lines=next_front_messages,
                variant="receipt",
            )
        )
...
def build_sheet_view(character):
    ...
    passive_items = [
        _build_sheet_passive_item(
            ...
            bonus_map=_combine_bonus_maps(race.get("bonuses", {}), race.get("trait_bonuses", {})),
        )
    ]
