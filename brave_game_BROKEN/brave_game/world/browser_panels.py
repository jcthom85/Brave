...
def build_chargen_panel(account, state):
    ...
    build_items = []
    race_key = state.get("race")
    if race_key:
        race_data = RACES.get(race_key, {})
        race_detail = race_data.get("summary", "")
        bonus_line = _format_stat_bonus_line(race_data.get("bonuses", {}))
        if bonus_line:
            race_detail = f"{race_detail} Bonuses: {bonus_line}" if race_detail else f"Bonuses: {bonus_line}"
        trait_line = _format_stat_bonus_line(race_data.get("trait_bonuses", {}))
        if trait_line:
            race_detail = f"{race_detail} Trait: {trait_line}" if race_detail else f"Trait: {trait_line}"
        build_items.append(_item(f"Race · {race_data.get('name', 'Not set')}", icon="forest", detail=race_detail))
    class_key = state.get("class")
    if class_key:
        class_data = CLASSES.get(class_key, {})
        class_lines = [class_data.get("summary", "")]
        class_lines.extend(_format_class_opening_abilities(class_data))
        class_detail = " ".join(line for line in class_lines if line)
        build_items.append(_item(f"Class · {class_data.get('name', 'Not set')}", icon="swords", detail=class_detail))
    ...
    if build_items:
        sections.append(_section("Build Details", "badge", build_items))
...
def build_quests_panel(character):
    ...
    if journal_mode == "active" and tracked_key:
        ...
        if remaining:
            ...
            tracked_lines.append(f"Next lead: {objective['description']}{suffix}")
        reward_text = _format_quest_reward_text(tracked_definition)
        if reward_text:
            tracked_lines.append(f"Reward: {reward_text}")
        if tracked_definition.get("reward_tip"):
            tracked_lines.append(f"Reward tip: {tracked_definition['reward_tip']}")
        if tracked_definition.get("next_step"):
            tracked_lines.append(f"Next lead: {tracked_definition['next_step']}")
