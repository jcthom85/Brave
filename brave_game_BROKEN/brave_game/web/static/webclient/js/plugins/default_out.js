...
let defaultout_plugin = (function () {
    ...
    var currentCombatPickerSnapshot = null;
    ...
    var buildMobileRoomUtilityMarkup = function () {
        ...
        var questPrimary = tracked
            ? (tracked.reward_tip ? "Reward tip: " + tracked.reward_tip : (objective || (tracked.next_step ? "Next lead: " + tracked.next_step : "Open the journal for full objectives.")))
            : "";
        var questSecondary = tracked && tracked.next_step ? "Next lead: " + tracked.next_step : (tracked && tracked.reward ? "Reward: " + tracked.reward : "");
        var routeCount = 0;
        if (navSection) {
        ...
        var questMarkup = (
            ...
            + (tracked
                ? "<div class='brave-view__mobile-quest-title'>" + escapeHtml(tracked.title || "") + "</div>"
                    + "<div class='brave-view__mobile-quest-line'>" + escapeHtml(questPrimary) + "</div>"
                    + (questSecondary ? "<div class='brave-view__mobile-quest-line'>" + escapeHtml(questSecondary) + "</div>" : "")
    ...
    var titleCaseVicinityLabel = function (value) {
        var raw = value == null ? "" : String(value);
        return raw.replace(/\b[a-z]/g, function (match) {
    ...
    var clearCombatFxGhost = function (ref, expectedNode) {
        if (ref) {
            ...
            if (!ghostNode || (expectedNode && ghostNode !== expectedNode)) {
                return;
            }
        ...
        if (expectedNode && expectedNode.parentNode) {
            expectedNode.parentNode.removeChild(expectedNode);
        }
    };
            ...
            sections: [
                {
                    ...
                    items: items.map(function (entry) {
                        var rawLabel = entry && (entry.text || entry.label) ? (entry.text || entry.label) : "";
                        var displayLabel = titleCaseVicinityLabel(rawLabel);
                        return {
                            text: displayLabel,
    ...
    var renderSceneCard = function (sceneData) {
        ...
        var objectiveItems = Array.isArray(tracked.objectives)
            ...
            : [];
        var guidanceItems = [];
        if (tracked.reward_tip) {
            guidanceItems.push({ text: "Reward tip: " + tracked.reward_tip, icon: "tips_and_updates" });
        }
        if (tracked.reward) {
            guidanceItems.push({ text: "Reward: " + tracked.reward, icon: "savings" });
        }
        if (tracked.next_step) {
            guidanceItems.push({ text: "Next lead: " + tracked.next_step, icon: "near_me" });
        }
    ...
    var renderMainView = function (viewData) {
        ...
        var combatPickerSnapshot = null;
        if (
            currentPickerData
            && currentViewData
            && currentViewData.variant === "combat"
        ) {
            combatPickerSnapshot = {
                data: currentPickerData,
                anchorRect: currentPickerAnchorRect,
            };
        }
        var preservePickerSheet = !!(
            currentPickerData
            && currentViewData
            && currentViewData.variant === "combat"
            && viewData.variant === "combat"
        );
        var preservedPickerData = preservePickerSheet ? currentPickerData : null;
        var preservedPickerAnchorRect = preservePickerSheet ? currentPickerAnchorRect : null;
        if (!preservePickerSheet) {
            clearPickerSheet();
        }
        ...
        var renderEntries = function (items, options) {
            return (
                "<div class='brave-view__entries'>"
                + (items || []).map(function (entry) {
                    var lead = "";
                    var backgroundIcon = entry && entry.background_icon ? entry.background_icon : "";
                    var reserveCombatLanes = !!(options && (options.variant === "party" || options.variant === "targets"));
                    if (entry && entry.badge) {
                    ...
                    var chips = (entry && Array.isArray(entry.chips)) ? entry.chips : [];
                    ...
                    var chipsMarkup = "";
                    if (reserveCombatLanes || chips.length) {
                        chipsMarkup = "<div class='brave-view__entry-chips" + (chips.length ? "" : " brave-view__entry-chips--empty") + "'>"
                            + chips.map(renderChip).join("")
                            + "</div>";
                    }
                    var bodyMarkup = "";
                    if (reserveCombatLanes || lines.length) {
                        bodyMarkup = "<div class='brave-view__entry-body" + (lines.length ? "" : " brave-view__entry-body--empty") + "'>"
                            + lines.map(function (line) {
        ...
        var renderSection = function (section) {
            ...
            if (kind === "pairs") {
                ...
                body = renderEntries(section.items || [], { variant: section.variant || "" });
            } else if (kind === "list") {
        ...
        var applyStickyMarkup = function () {
            ...
            setBodyState("view", viewData && viewData.variant ? viewData.variant : "");
                ...
                if (preservePickerSheet) {
                    currentPickerData = preservedPickerData;
                    currentPickerAnchorRect = preservedPickerAnchorRect;
                    renderPickerSheet();
                } else if (viewData.variant === "combat" && combatPickerSnapshot) {
                    currentPickerData = combatPickerSnapshot.data;
                    currentPickerAnchorRect = combatPickerSnapshot.anchorRect;
                    currentCombatPickerSnapshot = combatPickerSnapshot;
                    renderPickerSheet();
                }
