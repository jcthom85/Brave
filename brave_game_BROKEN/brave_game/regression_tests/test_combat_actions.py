...
class CombatActionPayloadTests(unittest.TestCase):
    def test_payload_marks_first_hour_reaction_tools(self):
        room = DummyRoom()
        warrior = DummyCharacter(
            ...
            room,
            "warrior",
            {"hp": 22, "mana": 0, "stamina": 18},
            {"max_hp": 24, "max_mana": 0, "max_stamina": 20},
            ["Defend", "Shield Bash"],
        )
        encounter = DummyEncounter(room, [warrior], [{"id": "e1", "key": "Goblin Cutter", "hp": 18, "max_hp": 24}])

        payload = build_combat_action_payload(encounter, warrior)
        defend = _action(payload["abilities"], "defend")
        shield_bash = _action(payload["abilities"], "shieldbash")

        self.assertEqual("guard", defend.get("reaction_role"))
        self.assertIn("GUARD", defend.get("text", ""))
        self.assertEqual("interrupt", shield_bash.get("reaction_role"))
        self.assertIn("INTERRUPT", shield_bash.get("text", ""))
