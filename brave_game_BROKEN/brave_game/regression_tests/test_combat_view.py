...
class CombatViewTests(unittest.TestCase):
    def test_targeted_ally_ability_uses_picker_targets(self):
        ...
        self.assertEqual("healing", dad_entry.get("background_icon"))
        self.assertIsNone(dad_entry.get("icon"))
        self.assertEqual(
            [("ATB", "45 / 100"), ("HP", "17 / 26"), ("STA", "9 / 12")],
            [(meter.get("label"), meter.get("value")) for meter in peep_entry.get("meters", [])],
        )
        self.assertEqual("swords", peep_entry.get("background_icon"))
    ...
    def test_pressure_section_lists_current_counterplay_for_winding_enemy(self):
        room = DummyRoom()
        warrior = DummyCharacter(
            7,
            "Dad",
            room,
            "warrior",
            {"hp": 20, "mana": 0, "stamina": 18},
            {"max_hp": 24, "max_mana": 0, "max_stamina": 20},
            ["Strike", "Defend", "Shield Bash"],
        )
        encounter = DummyEncounter(
            room,
            [warrior],
            [{"id": "e1", "key": "Ruk the Fence-Cutter", "hp": 64, "max_hp": 84, "template_key": "ruk_fence_cutter"}],
            atb_states={
                "p:7": {"phase": "ready", "gauge": 100, "ready_gauge": 100},
                "e:e1": {
                    "phase": "winding",
                    ...
                    "current_action": {
                        "kind": "enemy_attack",
                        "label": "Fence-Cleaver",
                        "counterplay_roles": ["interrupt", "guard"],
                        "counterplay_text": "Interrupt it or guard the hit cleanly.",
                    },
                },
            },
        ...
        view = build_combat_view(encounter, warrior)
        pressure = _section(view, "Pressure")

        self.assertEqual(
            ["Ruk the Fence-Cutter: Fence-Cleaver · Interrupt: Shield Bash · Guard: Defend"],
            [item.get("text") for item in pressure.get("items", [])],
        )
