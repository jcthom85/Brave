...
class JournalViewTests(unittest.TestCase):
    def test_journal_view_shows_tracked_then_active_regions(self):
        ...
        self.assertIn(f"Reward: 40 XP, 6 silver", tracked.get("items", [])[0].get("lines", []))
        self.assertIn(f"Next lead: {QUESTS[active_key]['next_step']}", tracked.get("items", [])[0].get("lines", []))
        ...
        goblin_lines = goblin_road.get("items", [])[0].get("lines")
        self.assertIn(f"Next lead: {QUESTS[second_active_key]['objectives'][0]['description']}", goblin_lines)
        self.assertIn("Reward: 70 XP, 10 silver, Wharfside Skewers", goblin_lines)
        self.assertIn(f"Reward tip: {QUESTS[second_active_key]['reward_tip']}", goblin_lines)
        self.assertIn(f"Next lead: {QUESTS[second_active_key]['next_step']}", goblin_lines)
