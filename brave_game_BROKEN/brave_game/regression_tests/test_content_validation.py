...
class ContentValidationTests(unittest.TestCase):
    ...
    def test_validator_reports_cross_domain_reference_errors(self):
        ...
        broken_characters = replace(
            registry.characters,
            classes={
                **registry.characters.classes,
                "warrior": {
                    **registry.characters.classes["warrior"],
                    "icon": "",
                },
            },
        )
        ...
        broken_registry = replace(
            registry,
            characters=broken_characters,
            items=broken_items,
            quests=broken_quests,
            world=broken_world,
            encounters=broken_encounters,
            dialogue=broken_dialogue,
            systems=broken_systems,
        )

        errors = validate_content_registry(broken_registry)

        self.assertTrue(any("is missing a material icon" in error for error in errors))
    ...
    def test_ranger_starter_loadout_includes_road_snack(self):
        registry = get_content_registry()
        ranger_loadout = registry.items.starter_loadouts.get("ranger", {})

        self.assertEqual("trail_mix_satchel", ranger_loadout.get("snack"))

    def test_druid_unlocks_verdant_guard(self):
        actions, passives, unknown = split_unlocked_abilities("druid", 10)

        self.assertIn("Verdant Guard", passives)
