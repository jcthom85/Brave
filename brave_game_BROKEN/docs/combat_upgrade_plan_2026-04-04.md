...
The first authored reaction pass should focus on the Goblin Road chapter instead of later bosses first.

Priority reads:

- `Road Wolf`: a simple early lunge that teaches "something is coming" before the player reaches the camp
- `Goblin Slinger`: a clear interrupt target
- `Goblin Cutter`: a clear interrupt-or-guard target
- `Ruk the Fence-Cutter`: the first boss windup the whole party should learn to call out and answer together

Priority answer tools:

- Warrior: `Defend`, `Shield Bash`
- Ranger: `Aimed Shot`
- Cleric: `Blessing`
- Mage: `Frost Bind`
- Rogue: `Feint`
- Paladin: `Guarding Aura`
