...
- quest completions should call out what just changed in the world, not only the XP math
- reward messaging should tell the player when food, silver, `shop`, `pack`, `sell`, or early `forge` decisions start mattering
...
- tracked quest details that show both the next lead and the expected reward
- no dead-end period after the tutorial or after a major quest completion

## Messaging Rules

For the first-hour ladder, quest completions should emit these in plain language whenever relevant:

- `Aftermath`: what changed because the player finished the beat
- `Reward tip`: why the payout matters right now
- `Next lead`: who or where to follow next

If one of those is missing at a key handoff, the opener is probably under-explaining the part the player actually needs.
...
- surfacing aftermath, reward tips, and next leads in the journal and room HUD
- making race and class selection explain the opening kit, perks, and early unlocks
